...
public class productController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(action != null && action.equals("showProducts")){     try {
            //to show list of products in product list page
        ...
        else if(action != null && action.equals("productController"))
        {
            try {
              ...
              //      System.out.println("line item reached here"+lineItem);
                } else if(quantity == 0){
                    Cart.removeItem(lineItem);
                }
                session.setAttribute("cartData", Cart);
         //       System.out.println("cart data id here"+cartData);
                url = "/cart.jsp";
                //}
        ...
        else if(action != null && action.equals("Update")){  try {
            //this part gets executed when an admin updates a product
          ...
          /**  try{
           ...
           try {
            ...
            if(decimalPlace > 0)
            {
                i = 1;
            String productid = request.getParameter("productid");
            product cartDataToDelete = null;
                try {
                    cartDataToDelete = ProductDB.getProduct(productid);
                } catch (ClassNotFoundException ex) {
                    Logger.getLogger(productController.class.getName()).log(Level.SEVERE, null, ex);
                } catch (SQLException ex) {
                    Logger.getLogger(productController.class.getName()).log(Level.SEVERE, null, ex);
        ...
        if (reply == JOptionPane.YES_OPTION) {
            ...
            if(i != 1){
                try {
                    ...
                    Product.setimageurl("images/new.jpg");
